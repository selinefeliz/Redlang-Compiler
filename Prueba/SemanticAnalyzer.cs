public class SemanticAnalyzer
{
    ...
    public SemanticAnalyzer()
    {
        ...
        // Register built-in classes (Standard Libraries)
        RegisterBuiltInClasses();
    }

    private void RegisterBuiltInClasses()
    {
        // En Redlang, System y Generics suelen ser bibliotecas estándar que no siempre tienen un archivo fisico
        string[] builtInClasses = { "System", "Generics" };
        foreach (var className in builtInClasses)
        {
            if (_globalScope.Lookup(className) == null)
            {
    ...
    // Load and register all symbols from the project first, ANALYZE
    public void LoadProject(List<ProgramNode> projectNodes)
    {
        // First pass: Register ClassSymbols for all classes in all files
        ...
        foreach (var node in projectNodes)
        {
            foreach (var classNode in node.Classes) RegisterClassMembers(classNode);
        }
    }

    // Analyze AST (Semantic Analysis) for the entire project
    public void AnalyzeProject(List<ProgramNode> projectNodes, ProgramNode entryProgram)
    {
        // 1. Build Global Scope (Classes and Members)
        LoadProject(projectNodes);

        // 2. Validate imports and Resolve Dependencies
        foreach (var program in projectNodes)
        {
             foreach (var useNode in program.Uses)
             {
                 if (_globalScope.Lookup(useNode.ClassName) == null)
                     throw new SemanticException($"Imposible to invoke use: Class '{useNode.ClassName}' not found or not compiled.");
                 
                 // In a more complex system, we might import the symbols into the file's local scope alias.
                 // For now, since we have a global symbol table, we just verify existence.
                 Console.WriteLine($"Confirmed import: {useNode.ClassName}"); 
             }
        }

        // 3. Analyze Definitions (Method bodies)
        foreach (var program in projectNodes)
        {
        ...
        if (_entryMethod == null || _entryClass == null)
            throw new SemanticException("No entry method found. Program cannot execute.");
            
        // Validate that the entry class only has the entry method (and no fields - optional rule from original code)
        ...
        {
             // Check if it's strictly the entry node that has the entry class? 
             // The original rule applied to the class containing the entry method.
    ...
    // Deprecated single-file analyze (keeps existing logic but delegates if possible, or we remove it)
    // We will keep a simplified version that wraps the single node in a list
    public void Analyze(ProgramNode program)
    {
        AnalyzeProject(new List<ProgramNode> { program }, program);
    }
    ...
    private string EvaluateExpression(ExpressionNode expr)
    {
        switch (expr)
        {
            ...
            case BinaryExpressionNode be:
                ...
                {
                    if (left != "b" || right != "b") 
                        throw new SemanticException($"logical operators require boolean operands (got '{left}' and '{right}').");
                    return "b";
                }
